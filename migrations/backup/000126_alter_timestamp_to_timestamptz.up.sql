-- =====================================================
-- MIGRATION: TIMESTAMP to TIMESTAMPTZ
-- =====================================================
-- Timezone: Asia/Jakarta (UTC+7)
--
-- Semua data existing diasumsikan sudah dalam timezone
-- Asia/Jakarta karena:
-- 1. Server berada di UTC+7
-- 2. Frontend mengirim data dalam UTC+7
-- 3. Data masuk sebagai nilai mentah tanpa timezone
--
-- Migrasi ini akan:
-- 1. Interpret data existing sebagai Asia/Jakarta
-- 2. Convert dan simpan dalam UTC internally
-- 3. Display sesuai session timezone
-- =====================================================

-- Set session timezone untuk migrasi
...
ALTER TABLE kategori_produk
    ...
    ALTER COLUMN deleted_at TYPE TIMESTAMPTZ USING deleted_at AT TIME ZONE 'Asia/Jakarta';
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'refresh_token') THEN
        ALTER TABLE refresh_token
            ALTER COLUMN created_at TYPE TIMESTAMPTZ USING created_at AT TIME ZONE 'Asia/Jakarta',
            ALTER COLUMN expired_at TYPE TIMESTAMPTZ USING expired_at AT TIME ZONE 'Asia/Jakarta',
...
ALTER TABLE buyer
    ...
    ALTER COLUMN deleted_at TYPE TIMESTAMPTZ USING deleted_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN last_login_at TYPE TIMESTAMPTZ USING last_login_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN email_verified_at TYPE TIMESTAMPTZ USING email_verified_at AT TIME ZONE 'Asia/Jakarta';
...
ALTER TABLE pesanan
    ...
    ALTER COLUMN ready_at TYPE TIMESTAMPTZ USING ready_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN shipped_at TYPE TIMESTAMPTZ USING shipped_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN completed_at TYPE TIMESTAMPTZ USING completed_at AT TIME ZONE 'Asia/Jakarta',
...
ALTER TABLE blog
    ...
    ALTER COLUMN updated_at TYPE TIMESTAMPTZ USING updated_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN deleted_at TYPE TIMESTAMPTZ USING deleted_at AT TIME ZONE 'Asia/Jakarta',
    ALTER COLUMN published_at TYPE TIMESTAMPTZ USING published_at AT TIME ZONE 'Asia/Jakarta';
...
DO $$
BEGIN
    RAISE NOTICE 'Migration completed: All TIMESTAMP columns converted to TIMESTAMPTZ';
    RAISE NOTICE 'Timezone: Asia/Jakarta (UTC+7)';
    RAISE NOTICE 'Data interpretation: Existing values treated as Asia/Jakarta time';
END $$;
